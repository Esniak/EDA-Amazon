...
# Limpieza robusta de la columna objetivo: elimina símbolos, comas y espacios
def clean_discount_price(value):
    if pd.isnull(value):
        return np.nan
    value = str(value)
    ...
    try:
        ...
        return np.nan
df[target] = df[target].apply(clean_discount_price)
# Mostrar ejemplos de valores problemáticos antes de filtrar
if df[target].isnull().sum() > 0:
    print(f"Ejemplos de valores no convertibles en '{target}':")
    print(df.loc[df[target].isnull(), target].head(10))
...
for cat in relevant_cats:
    ...
    if "price_per_rating" not in df_cat.columns:
        df_cat["price_per_rating"] = df_cat["actual_price"] / (df_cat["ratings"] + 0.1)
